...
def save_sp500_tickers():
    ...
    resp = requests.get('http://en.wikipedia.org/wiki/List_of_S%26P_500_companies')  # Wikipedia link to S&P 500 companies, can be changed to other links
    soup = bs.BeautifulSoup(resp.text, 'lxml')
    table = soup.find('table', {'class': 'wikitable sortable'})

    tickers1 = []
    
    # Several for loops allow any computer (given 16GB RAM) to save tickers into 3 different files. This is done to stop memory overuse
...
def get_data_from_yahoo(reload_sp500=False):
    ...
    # The same concept seen in save_sp500_tickers() is seen here, to get the data from Yahoo! Stocks API and organize into 3 separate .csv files
...
def visualize_data():
    ...
    Separated into 3 different graphs in case a computer running this can not handle 1 big graph."""
    ########################################################################################
...
def predictions(ticker):
    """Predicts spread using K-Nearest Neighbors and training based on the stock. Currently still in process of implementation
